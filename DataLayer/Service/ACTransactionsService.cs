...
namespace MicroApi.DataLayer.Service
{
                       ...
                       //cmd.Parameters.AddWithValue("@USER_ID", model.USER_ID ?? 0);
                        ...
                        foreach (var detail in model.DETAILS)
                        {
                            dt.Rows.Add(
                                ...
                                detail.DEPT_ID ?? 0, detail.STORE_ID ?? 0
                            );
                        }
    ...
    public JournalResponse UpdateJournal(JournalUpdateHeader header)
        ...
        public JournalListResponse GetJournalVoucherList(JVlistRequest request)
        {
            ...
            try
            {
                using (SqlConnection connection = ADO.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_AC_DEBIT_NOTE", connection))
                    {
                        ...
                        response.flag = 1;
                        response.Message = "Debit note Verified successfully.";
                    }
                }
            }
            catch (Exception ex)
        ...
        public DebitNoteListResponse GetDebitNoteList(DebitlistRequest request)
        {
