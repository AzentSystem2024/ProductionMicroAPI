    ...
    public class AccountService : IAccountService
    {
            ...
            try
            {
                using (var connection = ADO.GetConnection())
                {
                    ...
                    using (var cmd = new SqlCommand("SP_TB_AC_GROUP", connection))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@GROUP_NAME", account.GROUP_NAME);
                        cmd.Parameters.AddWithValue("@GROUP_SUPER_ID", account.GROUP_SUPER_ID);
                        cmd.Parameters.AddWithValue("@GROUP_TYPE", account.GROUP_TYPE);
                        cmd.Parameters.AddWithValue("@GROUP_LEVEL", account.GROUP_LEVEL);
            ...
            try
            {
                using (var connection = ADO.GetConnection())
                {
                    ...
                    using (var cmd = new SqlCommand("SP_TB_AC_GROUP", connection))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@GROUP_ID", account.GROUP_ID);
                        cmd.Parameters.AddWithValue("@GROUP_NAME", account.GROUP_NAME);
                        cmd.Parameters.AddWithValue("@ARABIC_NAME", account.ARABIC_NAME);
        ...
        public AccountListResponse GetLogList(int? id = null)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("SP_TB_AC_GROUP", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@ACTION", 0);
                cmd.Parameters.AddWithValue("@GROUP_ID", (object)id ?? DBNull.Value);
                cmd.Parameters.AddWithValue("@GROUP_NAME", DBNull.Value);
                cmd.Parameters.AddWithValue("@ARABIC_NAME", DBNull.Value);
                try
                {
