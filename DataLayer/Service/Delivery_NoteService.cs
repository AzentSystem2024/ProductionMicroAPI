...
namespace MicroApi.DataLayer.Service
{
    public class Delivery_NoteService:IDelivery_NoteService
    {
        ...
        {
            List<PENDINGSTOCKITEMS> items = new List<PENDINGSTOCKITEMS>();

            using (SqlConnection connection = ADO.GetConnection())
            {
                using (SqlCommand cmd = new SqlCommand("SP_TB_DN", connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ACTION", 3);
        ...
        public Int32 Save(DNSave deliverynote)
        {
            using (SqlConnection connection = ADO.GetConnection())
            {
                ...
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_TB_DN", connection, objtrans);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            DNSelect_Response response = new DNSelect_Response();
            DNSelect DN = new DNSelect();
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_TB_DN", connection))
                    {
                        ...
                        if (tbl.Rows.Count > 0)
                        {
                            ...
                            DN = new DNSelect
                            {
                                ...
                                NARRATION = firstRow["NARRATION"] != DBNull.Value ? firstRow["NARRATION"].ToString() : null,
                                DN_TYPE = firstRow["DN_TYPE"] != DBNull.Value ? (int?)Convert.ToInt32(firstRow["DN_TYPE"]) : null,
                                Details = new List<DNDetailSelect>()
                            };

                            foreach (DataRow dr in tbl.Rows)
                            {
                                var detail = new DNDetailSelect
                                {
                                    DETAIL_ID = dr["DETAIL_ID"] != DBNull.Value ? (int?)Convert.ToInt32(dr["DETAIL_ID"]) : null,
                                    ITEM_ID = dr["ITEM_ID"] != DBNull.Value ? (int?)Convert.ToInt32(dr["ITEM_ID"]) : null,
                                    UOM = dr["UOM"] != DBNull.Value ? dr["UOM"].ToString() : null,
                                    ...
                                    ITEM_CODE = dr["ITEM_CODE"] != DBNull.Value ? dr["ITEM_CODE"].ToString() : null,
                                    DESCRIPTION = dr["DESCRIPTION"] != DBNull.Value ? dr["DESCRIPTION"].ToString() : null,
                                    QTY_STOCK = dr["QTY_STOCK"] != DBNull.Value ? (double?)Convert.ToDouble(dr["QTY_STOCK"]) : null,
                                };
                                DN.Details.Add(detail);
                            }
        ...
        {
            DeliveryNoteListResponse response = new DeliveryNoteListResponse();
            List<DeliveryNoteList> data = new List<DeliveryNoteList>();

            using (SqlConnection connection = ADO.GetConnection())
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("SP_TB_DN", connection);
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            DeliveryNoteList item = new DeliveryNoteList
                            {
                                ID = dr["ID"] != DBNull.Value ? (int?)Convert.ToInt32(dr["ID"]) : null,
                                COMPANY_ID = dr["COMPANY_ID"] != DBNull.Value ? (int?)Convert.ToInt32(dr["COMPANY_ID"]) : null,
                                STORE_ID = dr["STORE_ID"] != DBNull.Value ? (int?)Convert.ToInt32(dr["STORE_ID"]) : null,
