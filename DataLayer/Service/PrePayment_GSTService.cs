...
{
    public class PrePayment_GSTService: IPrePayment_GSTService
    {
        public PrePayment_GSTResponse Save(PrePayment_GST model)
        {
            PrePayment_GSTResponse response = new PrePayment_GSTResponse();
            ...
            try
            {
                using (SqlConnection connection = ADO.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_TB_PREPAYMENT_GST", connection))
                    {
        ...
        {
            var response = new PrePayment_GSTListResponse
            {
                flag = 0,
                Message = "Failed",
                Data = new List<PrePayment_GSTListHeader>()
            };
            ...
            try
            {
                using (SqlConnection connection = ADO.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_TB_PREPAYMENT_GST", connection))
                    {
                        ...
                        {
                            Dictionary<int, PrePayment_GSTListHeader> headerMap = new Dictionary<int, PrePayment_GSTListHeader>();
                            ...
                            while (reader.Read())
                            {
                                ...
                                {
                                    headerMap[transId] = new PrePayment_GSTListHeader
                                    {
                                        TRANS_ID = transId,
                                        ...
                                        Details = new List<PrePayment_GSTListDetail>()
                                    };
                                }

                                // Add detail to header
                                headerMap[transId].Details.Add(new PrePayment_GSTListDetail
                                {
        ...
        public PrePayment_GSTListHeaderResponse GetPrePaymentById(int id)
        {
            var response = new PrePayment_GSTListHeaderResponse
            {
                flag = 0,
                Message = "Failed",
                Data = null
            ...
            try
            {
                using (SqlConnection con = ADO.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_TB_PREPAYMENT_GST", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@ACTION", 0);
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            PrePayment_GSTListHeader header = null;

                            while (reader.Read())
                            {
                                if (header == null)
                                {
                                    header = new PrePayment_GSTListHeader
                                    {
                                ...
                                // Add detail row
                                header.Details.Add(new PrePayment_GSTListDetail
                                {
        ...
        public PrePayment_GSTResponse Delete(int id)
        {
            ...
            try
            {
                using (var connection = ADO.GetConnection())
                {
                    ...
                    string procedureName = "SP_TB_PREPAYMENT_GST";
