    ...
    public class WorksheetItemPropertyService:IWorksheetItemPropertyService
    {
        ...
        public Int32 InsertPrice(WorksheetItem worksheet)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("USER_ID", worksheet.USER_ID);
                cmd.Parameters.AddWithValue("STORE_ID", worksheet.STORE_ID);
                cmd.Parameters.AddWithValue("NARRATION", worksheet.NARRATION ?? (object)DBNull.Value);
        ...
        public WorksheetItem GetPromotionItem(int id)
        {
            ...
            try
            {
                ...
                string strSQL2 = "SELECT TB_WORKSHEET_PROMOTION.*, TB_PROMOTION_SCHEMA.description, " +
                 ...
                 "TB_ITEM_CATEGORY.CAT_NAME, TB_ITEMS.BARCODE, TB_AC_TRANS_HEADER.NARRATION, TB_ITEMS.DESCRIPTION AS ITEM_DESCRIPTION " + 
                 "FROM TB_WORKSHEET_PROMOTION " + 
                 "LEFT JOIN TB_PROMOTION_SCHEMA ON TB_WORKSHEET_PROMOTION.PROMOTION_SCHEMA_ID = TB_PROMOTION_SCHEMA.ID " + 
                 "LEFT JOIN TB_ITEMS ON TB_WORKSHEET_PROMOTION.ITEM_ID = TB_ITEMS.ID " + 
                 "LEFT JOIN TB_ITEM_DEPARTMENT ON TB_ITEMS.DEPT_ID = TB_ITEM_DEPARTMENT.ID " + 
                 "LEFT JOIN TB_WORKSHEET ON TB_WORKSHEET_PROMOTION.WS_ID = TB_WORKSHEET.ID " + 
                 "LEFT JOIN TB_AC_TRANS_HEADER ON TB_WORKSHEET.TRANS_ID = TB_AC_TRANS_HEADER.TRANS_ID " + 
                 "LEFT JOIN TB_ITEM_CATEGORY ON TB_ITEMS.CAT_ID = TB_ITEM_CATEGORY.ID " + 
                 "LEFT JOIN TB_PROMOTION_LEVELS ON TB_WORKSHEET_PROMOTION.PROMOTION_LEVEL = TB_PROMOTION_LEVELS.ID " + 
                 "WHERE TB_WORKSHEET_PROMOTION.WS_ID = " + id; 
